...
- 메인페이지
    ...
    - 출퇴근 관리, 일정, 공지사항, 실시간 음원차트 확인 가능
- 전자결재 
    - 새결재 진행으로 기안서, 휴가신청서 작성 가능
    - 기안함에서 기안한 문서 리스트, 결재상태 확인 가능 
    - 결재함에서 결재 문서 리스트, 결재상태 확인 가능
    - 결재함에서 제목 클릭하면 문서 상세보기 이동, 결재 승인/반려 처리 가능 
    - 참조함에서 내가 참조된 리스트, 결재상태 확인 가능
    - 각 리스트 기안자, 제목으로 검색 가능 
- 근태관리 
    - 출결 관리 (출퇴근 시간 입력 가능)
    - 주별, 월별 누적 업무 시간 확인 가능 
- 연차관리 
    - 총 연차, 사용 연차, 잔여 연차수 확인 가능 
    - 연도별 연차 사용 내역 확인 가능 
- 일정관리
    - 임원, 경영지원부만 '전사 일정' 등록 가능
    - 부서별 '부서 일정' 등록 가능
- 인사관리
    <br/><경영지원부, 임원만 접근 가능>
    - 새직원 계정 생성, 수정 가능
    - 부서/팀 등록, 수정, 삭제 가능 
- 게시판
    - 전사 게시판, 부서 게시판 리스트, 작성, 상세보기, 수정, 삭제 가능
    - 부서 게시판은 로그인한 계정의 부서 게시판만 확인 가능 
- 공용자료실
    - 경영지원부서만 전사 파일 등록, 다운로드, 삭제 가능 
    - 부서별 파일 등록, 다운로드, 삭제 가능 (타부서는 접근 불가)
    - 사용자 본인이 작성한 파일만 삭제 가능 
- 사내 메일
    - 파일 첨부를 포함한 메일 작성 가능 (사내에서만 사용 가능)
    - 주소록에서 등록된 직원 정보를 가져와 '받는 사람'에 추가 가능 
    - 받은 메일함, 보낸 메일함 리스트(읽음/안읽음 필터링), 상세보기, 파일 다운로드 가능
    - 메일을 선택하여 삭제 가능
    - 전체 메일과 안읽은 메일의 수 확인 가능
- 알림 
    ...
    - 알림을 선택하여 삭제 가능 
    - 발생 후 7일이 지난 알림은 자동으로 삭제 
### 추가 서비스 
- 회의실 / 공연장 예약
    - 예약 신청하면 승인 대기 리스트에 추가 
    - 경영지원부에서 승인하면 예약 완료 리스트에 추가 
    - 경영지원부는 등록 대기 리스트에서 승인, 취소 가능 
- 아티스트관리
    - 아티스트 이름과 소속사 등록 가능
    - 아티스트 이름 클릭시 음반 목록 확인 가능
- 음반관리
   <br/> <마케팅부, 운영부만 접근 가능>
    - 음반 계약 현황 리스트, 음반명으로 검색 가능
    - 제목 클릭시 상세보기 이동 
    - 상세보기 - 생산 회차별 판매량/재고량 차트 확인, 계약서 다운로드 가능 
    - 상세보기 - 계약 취소, 추가 생산 클릭시 기안서 페이지로 이동
- 실시간 음원 차트  
    - 멜론 실시간 음원 차트 확인 가능
    - 소속 아티스트의 음원 차트만 필터링 가능 
- 저작권관리 (진행중)
    - 아티스트의 음원에 대한 저작권 관리
- 실적 관리 
    - 음반 실적 리스트, 음반명/아티스트명 검색 가능
    - 저작권 실적 관리 (진행중)
## 내가 구현한 부분
